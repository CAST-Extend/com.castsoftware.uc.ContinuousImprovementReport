...
class Report(ApplicationLevelExtension):
    ...
    def ConfigFilePath(self, centraldb, appname):
        ...
        self.config_file_path = os.path.join(self.rptdir, "ContinuousImprovement_%s.config" % (appname))
    ...
    def after_snapshot(self, application):    
        ...
        logging.info('***** Continuous ImprovementReport Generation *****')
        ...
        self.rptdir = os.path.join(self.get_plugin().intermediate, "ContinuousImprovement")
        ...
        if self.HasTwoSnapshots(central, appname):
            ...
            if return_code == 0:
                ...
                logging.info('* ' + msg)
                publish_report('Continuous Improvement Report', status, "List of the New Violations in the Education program between the last 2 snapshots", msg, detail_report_path=os.path.join(self.rptdir, latest_report))
            else:
                status = "KO"
                logging.warning('* Excel report generated : ERROR')
                publish_report('Continuous Improvement Report', status, "List of the New Violations in the Education program between the last 2 snapshots", 'Error in the report generation', detail_report_path='')
        else:
            status = "Warning"
            logging.warning('* Excel report generated : report not generated because there is only one snapshot')
            publish_report('Continuous Improvement Report', status, "Continuous Improvement Report", 'No report : only one snasphot', detail_report_path='')
        logging.info('***** End Continuous Improvement Report  *****')
        pass
