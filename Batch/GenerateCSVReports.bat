...
set requete=%requete%	FROM  	%DashboardService%.DSS_PORTF_TREE pt, %DashboardService%.CSV_DIAGDETAILS res, %DashboardService%.AAA_OBJECT_STATUSES st, %DashboardService%.VIEWER_ACTION_PLAN_TRIGGERS ed
...
set requete=%requete%	AND		ed.APPLICATION_ID	= pt.APP_ID
set requete=%requete%	AND		ed.METRIC_ID	= res.DIAG_ID
REM set requete=%requete%	AND		ed.ACTIVE		= 1
...
set requete=%requete%	%DashboardService%.AAA_OBJECT_STATUSES st, %DashboardService%.VIEWER_ACTION_PLAN_TRIGGERS ed
set requete=%requete%	WHERE	pt.APP_ID 			= (select distinct OBJECT_ID from %DashboardService%.DSS_OBJECTS o where o.OBJECT_TYPE_ID = -102 and upper(o.object_name) = upper('%ApplicationName%'))
set requete=%requete%	AND		pt.SNAPSHOT_ID 		= %newSnapshot%
...
set requete=%requete%	AND		ed.APPLICATION_ID	= pt.APP_ID
set requete=%requete%	AND		ed.METRIC_ID		= res1.DIAG_ID
REM set requete=%requete%	AND		ed.ACTIVE			= 1
...
set requete=%requete% CAST(dmtt.metric_critical as varchar), CAST(dmtt.aggregate_weight as varchar), replace(dmd.metric_description,',', ''), 
set requete=%requete% CAST(COALESCE(nv.NumNew, 0) as varchar), CAST(COALESCE(fv.NumFixed, 0) as varchar), ed.tag, ed.action_def, CAST(ed.sel_date as varchar)
...
set requete=%requete% %DashboardService%.dss_metric_descriptions dmd, %DashboardService%.viewer_action_plan_triggers ed,
...
set requete=%requete% and ed.metric_id = dmd.metric_id
set requete=%requete% UNION
set requete=%requete% select CAST(ed.metric_id as varchar),
set requete=%requete% CAST(dmtt.metric_critical as varchar), CAST(dmtt.aggregate_weight as varchar), replace(dmd.metric_description,',', ''), '0' , '0' , ed.tag, ed.action_def , CAST(ed.sel_date as varchar)
set requete=%requete% from	%DashboardService%.viewer_action_plan_triggers ed, %DashboardService%.dss_metric_descriptions dmd, 
set requete=%requete% (select metric_id, metric_critical, aggregate_weight from %DashboardService%.dss_metric_type_trees a where aggregate_weight = (select max(aggregate_weight) from %DashboardService%.dss_metric_type_trees b where a.metric_id = b.metric_id)) dmtt
set requete=%requete% where ed.metric_id not in (select distinct diag_id from %DashboardService%.AAA_RECENT_VARIATIONS)
set requete=%requete% and ed.metric_id = dmd.metric_id
set requete=%requete% and dmd.language='ENGLISH'
set requete=%requete% and dmd.description_type_id = 0
set requete=%requete% and dmtt.metric_id = dmd.metric_id
set requete=%requete% order by 5 desc , 6 desc, 2 desc, 3 desc;"
