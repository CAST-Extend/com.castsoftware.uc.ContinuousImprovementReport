...
if not exist "%ReportPath%" mkdir "%ReportPath%"	> %ReportLogPath%\EducationReport_%ApplicationName%_%CaptureDate%.log

echo Generating the CSV Reports
echo Generating the CSV Reports  >> %ReportLogPath%\EducationReport_%ApplicationName%_%CaptureDate%.log

call %BatchPath%\GenerateCSVReports.bat %ReportPath% %BatchPath% %DashboardService% %ApplicationName% %CaptureDate% %CSSServer% %userLogin% %database% %portNumber% %EDURL% %CCSPSQL% >> %ReportLogPath%\EducationReport_%ApplicationName%_%CaptureDate%.log
if %errorlevel% NEQ 0 echo FAILED CSV Generation & goto :AutomationFailed

echo SUCCESSFUL CSV Generation
echo SUCCESSFUL CSV Generation >> %ReportLogPath%\EducationReport_%ApplicationName%_%CaptureDate%.log

echo Zipping the CSV reports >> %ReportLogPath%\EducationReport_%ApplicationName%_%CaptureDate%.log
%SevenZip% a -tzip "%ReportPath%\%ApplicationName%_EducationReport_%CaptureDate%.zip" "%ReportPath%\%ApplicationName%_*_%CaptureDate%.csv" >> %ReportLogPath%\EducationReport_%ApplicationName%_%CaptureDate%.log
if %errorlevel% NEQ 0 goto :AutomationFailed

echo SUCCESSFUL CSV Zip >> %ReportLogPath%\EducationReport_%ApplicationName%_%CaptureDate%.log
...
echo Generating Excel Report on %ApplicationName% in folder %ReportPath%  >> %ReportLogPath%\EducationReport_%ApplicationName%_%CaptureDate%.log
...
echo SUCCESSFUL Excel Report Generation >> %ReportLogPath%\EducationReport_%ApplicationName%_%CaptureDate%.log

echo Removing the CSV Reports >> %ReportLogPath%\EducationReport_%ApplicationName%_%CaptureDate%.log
del "%ReportPath%\%ApplicationName%_*_%CaptureDate%.csv" >> %ReportLogPath%\EducationReport_%ApplicationName%_%CaptureDate%.log
echo SUCCESFUL CSV Reports Removal >> %ReportLogPath%\EducationReport_%ApplicationName%_%CaptureDate%.log
...
echo FAILED Report Generation >> %ReportLogPath%\EducationReport_%ApplicationName%_%CaptureDate%.log
